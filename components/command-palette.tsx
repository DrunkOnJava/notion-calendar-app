...
export function CommandPalette({ isOpen, onClose, onCommand }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center pt-[20vh] z-50" onClick={onClose}>
      <div
        className="bg-[#1c1c1c] rounded-lg w-[600px] overflow-hidden border border-[#2a2a2a] shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[#2a2a2a]">
          ...
          <input
            ...
            autoFocus
          />
          <kbd className="px-2 py-1 text-xs bg-[#2a2a2a] rounded text-[#9a9a9a]">ESC</kbd>
        </div>
        ...
        <div className="max-h-[400px] overflow-auto">
          {Object.entries(groupedCommands).map(([category, cmds]) => (
            <div key={category}>
              <div className="px-4 py-2 text-xs text-[#6b6b6b] font-medium">{category}</div>
              {cmds.map((cmd, idx) => {
        ...
        {/* Footer */}
        <div className="px-4 py-2 border-t border-[#2a2a2a] flex items-center justify-between text-xs text-[#6b6b6b]">
          <div className="flex items-center gap-4">
            <span>
              <kbd className="px-1.5 py-0.5 bg-[#2a2a2a] rounded">↑</kbd>
              <kbd className="px-1.5 py-0.5 bg-[#2a2a2a] rounded ml-1">↓</kbd> Navigate
            </span>
            <span>
              <kbd className="px-1.5 py-0.5 bg-[#2a2a2a] rounded">Enter</kbd> Select
            </span>
          </div>
        </div>
      </div>
